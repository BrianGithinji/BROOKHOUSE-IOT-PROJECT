        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 sm:py-6">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-6 min-w-0 flex-1">
              <img src={logo} alt="Brookhouse Schools" className="h-6 sm:h-16 flex-shrink-0" />
              <div className="border-l border-slate-300 pl-2 sm:pl-6 min-w-0">
                <h1 className="text-slate-900 text-xs sm:text-2xl font-medium truncate">Brookhouse Smart Campus Dashboard</h1>
                <p className="text-slate-600 text-xs sm:text-lg hidden sm:block">Real-time Environmental Monitoring</p>
              </div>
            </div>
            <div className="flex items-center gap-1 sm:gap-6 flex-shrink-0">
              <button
                onClick={() => setCurrentPage('dashboard')}
                className={`flex items-center gap-1 sm:gap-3 px-2 py-1.5 sm:px-6 sm:py-3 rounded-md transition-colors text-xs sm:text-lg ${
                  currentPage === 'dashboard'
                    ? 'bg-emerald-100 text-emerald-700'
                    : 'text-slate-600 hover:bg-slate-100'
                }`}
              >
                <LayoutDashboard className="w-3 h-3 sm:w-6 sm:h-6" />
              ...
              >
                <BookOpen className="w-3 h-3 sm:w-6 sm:h-6" />
                <span className="hidden sm:inline">Setup Guide</span>
              </button>
              {currentPage === 'dashboard' && (
                <div className="flex items-center gap-1 sm:gap-3">
                  <div className="w-1.5 h-1.5 sm:w-3 sm:h-3 bg-emerald-500 rounded-full animate-pulse"></div>
                  <span className="text-xs sm:text-lg text-slate-600 hidden sm:inline">Live</span>
                </div>
              )}
            </div>
          </div>
